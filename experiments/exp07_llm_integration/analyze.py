...
def analyze(results_path: Path, outdir: Path):
    ...
    # Structure: {(D, K, H): {N: [accuracy_values, capacity_units, N_over_D, tokens_per_sec_values]}}
    data_by_config = defaultdict(lambda: defaultdict(lambda: {
        "bbpm": [], "baseline": [], "oracle": [], "capacity_units": None, "N_over_D": None, "tokens_per_sec": []
    }))
    ...
    for seed_key, seed_data in seeds_data.items():
        for config_key, config_data in seed_data.items():
            ...
            if data_by_config[key][N]["capacity_units"] is None:
                data_by_config[key][N]["capacity_units"] = config_data.get("capacity_units", config_data.get("N_over_D", 0.0))
    ...
    for (D, K, H) in sorted(data_by_config.keys()):
        ...
        for N in sorted(config_data.keys()):
            vals = config_data[N]
            if len(vals["bbpm"]) > 0:
                # Prefer capacity_units, fallback to N_over_D
        ...
        if len(capacity_units_vals) > 0:
            ...
            if num_seeds >= 3:
                ...
                ax1.plot(capacity_units_vals, bbpm_means, f"{color}-o", label=label, linewidth=2)

            # Baseline and oracle (plot once for first config)
            if color_idx == 0:
                if num_seeds >= 3:
                    ax1.errorbar(
                        capacity_units_vals, baseline_means, yerr=baseline_stds,
                        fmt="r--o", label="Baseline (Window)", linewidth=2, capsize=3
                    )
                    if len(oracle_means) > 0:
                        ax1.errorbar(
                            capacity_units_vals, oracle_means, yerr=oracle_stds,
                            fmt="g--o", label="Oracle (Upper Bound)", linewidth=2, capsize=3, alpha=0.7
                        )
                else:
                    ax1.plot(capacity_units_vals, baseline_means, "r--o", label="Baseline (Window)", linewidth=2)
                    if len(oracle_means) > 0:
                        ax1.plot(capacity_units_vals, oracle_means, "g--o", label="Oracle (Upper Bound)", linewidth=2, alpha=0.7)
    ...
    ax1.set_title("Exp 7: LLM Integration - Accuracy vs Capacity Units")
    ax1.legend()
